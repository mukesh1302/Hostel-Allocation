...
?>
  ...
  <table class="table table-hover">
    ...
    <?php
	if ($result1 === false) {
		echo "<script type='text/javascript'>alert('Database query failed: " . mysqli_error($conn) . "');</script>";
...
if (isset($_POST['submit'])) {
    $result1 = mysqli_query($conn, $query1);  // Fetch all applications

    while ($row1 = mysqli_fetch_assoc($result1)) {
        $student_id = $row1['Student_id'];  // Get the student ID

        // Find the minimum room number for each student
		$query2 = "SELECT * FROM Room WHERE Allocated = '0' AND Hostel_id = '$hostel_id' ORDER BY Room_No ASC LIMIT 1";

        $result2 = mysqli_query($conn, $query2);

        // If no rooms are available, exit the loop and show an alert
        if (!$result2 || mysqli_num_rows($result2) == 0) {
            echo "<script type='text/javascript'>alert('Rooms not available')</script>";
            break; // Exit the loop if no rooms are available
        }
        ...
        // Update the Application table to assign the room to the student
        $query3 = "UPDATE Application SET Application_status = '0', Room_No = '$room_no' WHERE Student_id = '$student_id'";
        $result3 = mysqli_query($conn, $query3);

        if ($result3) {
            // Update the Student table to assign the hostel and room
            $query4 = "UPDATE Student SET Hostel_id = '$hostel_id', Room_id = '$room_id' WHERE Student_id = '$student_id'";
            $result4 = mysqli_query($conn, $query4);

            if ($result4) {
                // Mark the room as allocated in the Room table
                $query5 = "UPDATE Room SET Allocated = '1' WHERE Room_id = '$room_id'";
                $result5 = mysqli_query($conn, $query5);

                if ($result5) {
                    echo "<script type='text/javascript'>alert('Room {$room_no} allocated successfully to student ID: {$student_id}')</script>";
                } else {
                    echo "<script type='text/javascript'>alert('Failed to update Room allocation')</script>";
                }
            } else {
                echo "<script type='text/javascript'>alert('Failed to update Student room allocation')</script>";
            }
        } else {
            echo "<script type='text/javascript'>alert('Failed to update Application')</script>";
        }
    }
}
